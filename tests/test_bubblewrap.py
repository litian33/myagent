    ...
    result = executor.execute(
        CommandRequest(
            argv=(
                "/usr/bin/python3",
                "-c",
                ("from pathlib import Path;Path('generated.txt').write_text('hello')"),
            ),
            cwd=tmp_path,
            timeout_seconds=10,
        )
...
def test_git_config_is_hidden(
    ...
    (git_dir / "config").write_text("https://user:token@example.com/repo.git")
    ...
    result = executor.execute(
        CommandRequest(
            argv=(
                "/usr/bin/python3",
                "-c",
                ("from pathlib import Path;print(Path('.git/config').read_text())"),
            ),
            cwd=workspace_root,
            timeout_seconds=10,
        )
